...
A later real export caused a confirmed 512 MiB OOM restart and temporary 502 at 23:51 UTC on 12 September. The export recovered automatically. The deployed fix reduces full-resolution encoder buffering. Re-exporting the actual three-second selection completed with all 180 frame timestamps, portrait resolution and audio preserved, a 333 MiB peak including operator checks, no OOM, and 90 successful HTTP health checks. The original and earlier completed export hashes remain intact. Output size increased from 3.18 MB to 5.96 MB; this trades compression efficiency for memory headroom. Hosting and provider allocations are unchanged. These results qualify this clip, not arbitrary footage or concurrent traffic. [Incident, all review findings and validation](EXPORT-MEMORY-REVIEW.md).
