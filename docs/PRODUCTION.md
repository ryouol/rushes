...
Local access is running at `http://localhost:3741`, with the existing signed-in workspace and all seven original videos visible. Those seven videos still need analysis after their historical missing-key failures; the key is configured now, but they were not reprocessed during this repair while the hosting footprint is being reduced to fit the confirmed combined monthly budget. The current Render plans, disk, instance count, environment values and provider allowance were preserved. Paid processing and real Google login were not repeated on this deployment.


## Static frontend candidate

Commits `38c32ca` and `7cf600e` remove the production Node process, serve built pages through the shared Python API/worker runtime, and run one media activity at a time. The local production launcher uses this architecture. Builds verify an explicit public configuration manifest; local development keeps its Next server and uses the same public API security boundary.

The final isolated suite passed 141 tests and the canonical local frontend passed 27 browser regressions. A frozen-source 512 MiB/0.5 CPU container passed 4K synthetic upload, interrupted preparation, same-job recovery, source/session preservation and all supervisor crash/shutdown checks. Three muted derivatives retaining actual uploaded video streams also prepared successfully with concurrent sign-in in a separate prototype. These are short-input checks; the accepted 8K upper limit and representative long media are not qualified at the smaller size. No plan downgrade, paid processing or production deployment occurred in this stage. [All twenty review findings and dispositions](STATIC-FRONTEND-REVIEW.md), [validation scope and cleanup](validation/static-frontend.json).
